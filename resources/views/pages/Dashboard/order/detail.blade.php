                            ...
                            <div class="grid gap-5 md:grid-cols-12">
                                        ...
                                        <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                                            <div class="flex mt-2 flex-nowrap">
                                                ...
                                                @forelse ($thumbnail as $item)
                                                    ...
                                                    src="{{ url(Storage::url($item->thumbnail)) }}" alt="thumbnail service" class="inline-block w-24 mr-2 rounded-lg cursor-pointer">
                                                @empty
                                                    {{-- empty --}}
                                                @endforelse
                                                </div>
                                    ...
                                    <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                                        ...
                                        <div class="px-4 pt-4 pb-2 features-list">
                                            <ul class="mb-4 text-sm list-check">
                                                @forelse ($advantage_service as $advantage_service_item)
                                                    <li class="pl-10 my-4">- {{ $advantage_service_item->advantage ?? '' }}</li>
                                                @empty
                                                    {{-- empty --}}
                                                @endforelse
                                            </ul>
                                ...
                                <div class="p-4 lg:col-span-6 md:col-span-12">
                                    @forelse ($tagline as $tagline_item)
                                        <span class="inline-flex justify-center px-3 py-2 mb-2 text-xs font-medium text-gray-700 bg-gray-100 border border-transparent rounded-lg hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">{{ $tagline_item->tagline ?? '' }}</span>
                                    @empty
                                        {{-- empty --}}
                                    @endforelse
                                </div>
                                <div class="p-4 md:text-right lg:col-span-6 md:col-span-12">
                                    ...
                                    <a href="{{ route('member.service.edit', $service->id) }}" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white border border-transparent rounded-lg shadow-sm bg-serv-email hover:bg-serv-email-text focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-serv-email">
                                        Edit Service
                                    </a>
                                </div>
                            </div>
...
@push('after-script')
    <script>
        function gallery() {
            return {
                featured: '{{ url(Storage::url($service->thumbnail_service[0]->thumbnail)) }}',
                active: 1,
